...
class simple_links extends SL_post_type_tax{
	...
	function __construct(){
	    $this->meta_box_descriptions = array( 'web_address' 	 => __('Example','wp-simple-links').': <code>http://wordpress.org/</code> '.__('DO NOT forget the','wp-simple-links').' <code>http:// or https://</code>',
											  'description'	     => __('This will be shown when someone hovers over the link, or optionally below the link','wp-simple-links').'.',
											  'target'           => __('Choose the target frame for your link','wp-simple-links').'.',
											  'additional_fields'=> __('Values entered in these fields will be available for shortcodes and Widgets','wp-simple-links').' '
										);
	...
	function shortcode( $atts ){
		...
		if( $atts['title'] != false ){
			$output .= sprintf('<h4 class="wp-simple-links-title">%s</h4>', $atts['title'] );
			
		}
		
		//Start the UL
		if( $atts['id'] ){
		    $output .= '<ul class="wp-simple-links-list" id="' . $atts['id'] . '">';
		} else {
		    $output .= '<ul class="wp-simple-links-list">';
		}
		...
		$links['title'] = $atts['title'];
			...
			foreach( $links as $link ){
				...
				$output .= '<li class="wp-simple-links-shortcode-item">';
	...
	if( isset( $this->meta_box_descriptions['additional_fields'] ) ){
		   ...
		   if( current_user_can($simple_links_admin_func->cap_for_settings)){
		   	       echo '<p>'.__('You may add additonal fields which will be available for all links in the ', 'wp-simple-links' ).'
					 				<a href="/wp-admin/edit.php?post_type=simple_link&page=simple-link-settings">'.__('Settings', 'wp-simple-links' ).'</a>
			  														</p>';
	...
	function target_meta_box_output($post){
		...
		<code>_blank</code> &minus; <?php _e('new window or tab','wp-simple-links');?>.</label></p>
		<p><label for="link_target_top" class="selectit">
		<input id="link_target_top" type="radio" name="target" value="_top" <?php checked( get_post_meta( $post->ID, 'target', true), '_top' );?>>
		<code>_top</code> &minus; <?php _e('current window or tab, with no frames','wp-simple-links');?>.</label></p>
		<p><label for="link_target_none" class="selectit">
		<input id="link_target_none" type="radio" name="target" value="" <?php checked( get_post_meta( $post->ID, 'target', true), '' );?>>
		<code>_none</code> &minus; <?php _e('same window or tab','wp-simple-links');?>.</label></p>
		<?php 
		...
		<input id="link_target_nofollow" type="checkbox" name="link_target_nofollow" value="1" 
		      ...
		      &nbsp; <?php _e('Add a','wp-simple-links');?> <code>nofollow</code> <?php _e('rel to this link','wp-simple-links');?> 
		</p>
		<?php
	...
	function link_meta_box_output($post, $box){
        ...
        if( $box != 'description' ){
                 printf('<input type="text" name="%s" value="%s" size="100" class="wp-simple-links-input">', $box, get_post_meta( $post->ID, $box, true ) );
        } else {
               printf('<textarea name="%s" class="wp-simple-links-input">%s</textarea>', $box, get_post_meta( $post->ID, $box, true ) );
        }
	...
	function link_categories(){
		self::register_taxonomy( 'simple_link_category', 'simple_link', array(
															'labels' => array(
																		'name'             => __('Link Categories','wp-simple-links'),
																	    'singular_name'    => __('Link Category','wp-simple-links'),
																		'all_items'        => __('Link Categories','wp-simple-links'),
																		'menu_name'        => __('Link Categories','wp-simple-links'),
																		'add_new_item'     => __('Add New Category','wp-simple-links'),
																		'update_item'      => __('Update Category','wp-simple-links')	
															),
	...
	function post_type(){
		$this->register_post_type( 'simple_link' , array(
				                                           ...
				                                           'labels'    => array(
				                                           		           'singular_name' =>  __('Link','wp-simple-links'),
				                                           				   'all_items'     =>  __('All Links','wp-simple-links'),
				                                           				   'new_item_name' =>  __('New Link','wp-simple-links'),
				                                           		           'add_new_item'  =>  __('Add Link','wp-simple-links'),
				                                           				   'add_new'       =>  __('Add Link','wp-simple-links'),
				                                           				   'view_item'     =>  __('View Link','wp-simple-links')
				                                           		),
															'hierachical' => false,
